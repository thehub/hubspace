		...
		<div class="dataBoxContent">
			<table class="detailTable data invoice_history" cellpadding="" cellspacing="0">
				...
				<tr py:for='invoice in Invoice.selectBy(user=object.id).orderBy("sent")' class="$odd_or_even()">
					...
					<td py:if="invoice.sent" class="resent_${object.id}">${dateconverter.from_python(invoice.sent)}&nbsp;&nbsp;<a py:if='permission_or_owner(invoice.location, None, "manage_invoices")' id="resent_invoice_${object.id}" class="${invoice.id}">re-send invoice</a></td> 
                                        <td py:if="not invoice.sent" class="unsent" id="unsent_${object.id}"><c py:if='not permission_or_owner(invoice.location, None, "manage_invoices")' py:strip="True">unsent</c><a py:if='permission_or_owner(invoice.location, None, "manage_invoices")' id="unsent_invoice_${object.id}" class="${invoice.id}">send invoice</a></td>
					<td>${c2s(invoice.amount)+ " "+ invoice.location.currency}</td>
					<td>
                                           <a py:if="invoice.sent or not permission_or_owner(invoice.location, None, 'manage_invoices')" class="view_invoice" id="invoice_${invoice.id}">As HTML</a>
                                           <a class="view_invoice" id="invoice_${invoice.id}" py:if="not invoice.sent and permission_or_owner(invoice.location, None, 'manage_invoices')">Modify</a>&nbsp;&nbsp;
                                           <a class="remove_invoice" id="removeInvoice_${invoice.id}" py:if="not invoice.sent and permission_or_owner(invoice.location, None, 'manage_invoices')">Remove</a> &nbsp;&nbsp;
